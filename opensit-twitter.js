...
const session = `query getSession($id: ID!) {
  session(where: {id: $id}) {
    id
    title
    speakers {
...
request(
  process.env.GRAPHCMS_ENDPOINT, allSessions
).then(
  data => {
    let sessionId;
    ...
    // or use a random 
      const randomSession = Math.floor(Math.random() * data.sessions.length);
      sessionId = data.sessions[randomSession].id;
    }
